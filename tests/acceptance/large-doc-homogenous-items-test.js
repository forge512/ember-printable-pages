...
module("Acceptance | large doc homogenous items", function (hooks) {
  ...
  test("re-rendering", async function (assert) {
    ...
    assert.dom("[data-test-page='1'] [data-test-section-item]").exists({ count: 12 });
  });
  ...
  test("column count", async function (assert) {
    ...
    assert.ok(item.offsetWidth > 700, "sections are > 700px when in single column mode");

    await fillIn("[data-test-column-count]", 2);
    item = find("[data-test-section-item]");
    assert.ok(item.offsetWidth > 330, "sections are about 340px when in dual column mode");
    assert.ok(item.offsetWidth < 350, "sections are about 340px when in dual column mode");

    await fillIn("[data-test-column-count]", 3);
    item = find("[data-test-section-item]");
    assert.ok(item.offsetWidth < 250, "sections are less than 250px when in 3 column mode");
  });
